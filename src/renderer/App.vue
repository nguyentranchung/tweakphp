<script setup lang="ts">
  ...
  import { useColorSchemeStore } from './stores/color-scheme.js'
  import { useExecuteStore } from './stores/execute.js'
  ...
  onMounted(async () => {
    ...
    window.ipcRenderer.receive('init.reply', async (e: any) => {
      isAppReady.value = true
      settingsStore.settings = e.settings
    })
    window.ipcRenderer.receive('source.open.reply', (e: any) => {
    ...
    window.ipcRenderer.receive('client.execute.reply', (e: any) => {
      events.dispatchEvent(new CustomEvent('client.execute.reply', { detail: e }))
    })
    window.ipcRenderer.receive('client.info.reply', (e: any) => {
      events.dispatchEvent(new CustomEvent('client.info.reply', { detail: e }))
    })
    window.ipcRenderer.receive('ssh.connect.reply', (e: any) => {
